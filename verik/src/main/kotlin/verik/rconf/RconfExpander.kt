...
internal class RconfExpander {

    companion object {

        fun expand(list: RconfList): List<RconfEntry> {
            val entries = ArrayList<RconfEntry>()
            validate_name(list.name)
            expand_recursive(list, list.name, entries)
            return entries
        }

        private fun expand_recursive(list: RconfList, base: String, entries: ArrayList<RconfEntry>) {
            ...
            for (entry in list.entries) {
                ...
                entries.add(RconfEntry("$base/${entry.name}", entry.data, entry.count))
            }
        }
