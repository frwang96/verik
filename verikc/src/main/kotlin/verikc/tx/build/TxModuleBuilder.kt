...
object TxModuleBuilder {
    ...
    fun build(module: SvModule, builder: TxSourceBuilder) {
        if (module.ports.isEmpty()) {
            ...
            indent(builder) {
                val alignedLines = module.ports.map { TxPortBuilder.build(it) }
        ...
        indent(builder) {
            ...
            if (module.primaryProperties.isNotEmpty()) {
                builder.appendln()
                val alignedLines = module.primaryProperties.map { TxPrimaryPropertyBuilder.build(it) }
                val alignedBlock = TxAlignedBlock(alignedLines, ";", ";")
                alignedBlock.build(builder)
            }
