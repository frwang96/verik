...
object TxBuilderExpressionBase {
    ...
    fun build(expression: SvExpression, isAutomatic: Boolean, builder: TxSourceBuilder) {
        if (expression is SvExpressionControlBlock) {
            buildControlBlock(expression, isAutomatic, builder)
        } else {
    ...
    private fun buildControlBlock(
        expression: SvExpressionControlBlock,
        isAutomatic: Boolean,
        builder: TxSourceBuilder
    ) {
            ...
            SvControlBlockType.IF_ELSE -> {
                ...
                if (expression.blocks[1].expressions.size == 1) {
                    ...
                    if (chainedExpression is SvExpressionControlBlock
                        ...
                        builder.append("else ")
                        build(chainedExpression, isAutomatic, builder)
                    } else {
                        builder.append("else ")
