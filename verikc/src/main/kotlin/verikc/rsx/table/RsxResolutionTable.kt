...
class RsxResolutionTable {
    ...
    private val resolutionEntriesMap = ConcurrentHashMap<Symbol, List<RsxResolutionEntry>>()

    fun addFile(fileSymbol: Symbol, resolutionEntries: List<RsxResolutionEntry>) {
    ...
    fun addScope(scopeSymbol: Symbol, parentSymbol: Symbol, line: Line) {
        ...
        val resolutionEntry = RsxResolutionEntry(listOf(scopeSymbol), listOf())
        resolutionEntriesMap[scopeSymbol] = listOf(resolutionEntry) + parentResolutionEntries
    }

    fun resolutionEntries(scopeSymbol: Symbol, line: Line): List<RsxResolutionEntry> {
